...
**Last updated:** 24 September 2026, 04:05 UTC. Every count in this file is a snapshot: re-run the command in the right-hand column before quoting it.
...
|---|---|---|
| Contracts | ✅ season 2 world `0xd239713b…` live on chain 97; registry & treasury unchanged, so reputations and guardian policies carried over | `npm run readback`, `node tools/probe-surface.mjs` |
| Rules under test | ✅ **92 passed / 0 failed** (Solidity) + **11** policy tests | `forge test`, `npm run test:policy` |
| Autonomous runtime | ✅ running; 78 completed actions unattended across two worlds as of 24 Sep | `agent/history/actions.jsonl`, `npm run world`, `npm run health` |
